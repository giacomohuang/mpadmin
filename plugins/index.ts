...
export function createSvgIconsPlugin(opt: ViteSvgIconsPlugin): Plugin {
  ...
  return {
    ...
    async load(id, ssr) {
      ...
      const { code, idSet } = await createModuleCode(cache, svgoOptions as OptimizeOptions, options)
      if (isRegister) {
        return code
      }
      if (isClient) {
...
export async function createModuleCode(cache: Map<string, FileStats>, svgoOptions: OptimizeOptions, options: ViteSvgIconsPlugin) {
 ...
 * @param options
 */
export async function compilerIcons(cache: Map<string, FileStats>, svgOptions: OptimizeOptions, options: ViteSvgIconsPlugin) {
...
export async function compilerIcon(file: string, symbolId: string, svgOptions: OptimizeOptions): Promise<string | null> {
  if (!file) {
    return null
  }
